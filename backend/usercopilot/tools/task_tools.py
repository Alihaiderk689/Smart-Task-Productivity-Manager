...
class CreateTaskTool(BaseTool):
    ...
    def run(self, **kwargs) -> ToolResult:
        ...
        # committed above, so a failure here must not be reported as "task
        # creation failed" (it isn't; the task exists).
        # NotificationService.schedule_reminders never raises (see its own
        # docstring) -- it returns whether scheduling actually succeeded,
        # which is all this needs to pick the right status_note.
        scheduled = NotificationService.schedule_reminders(task)
        status_note = "created, reminders scheduled" if scheduled else "created, but reminders could not be scheduled"
...
class UpdateTaskTool(BaseTool):
    ...
    def run(self, **kwargs) -> ToolResult:
        ...
        if "start_time" in payload or "end_time" in payload:
            ...
            scheduled = NotificationService.schedule_reminders(task)
            status_note = "updated, reminders rescheduled" if scheduled else "updated, but reminders could not be rescheduled"
        else:
            status_note = "updated"
